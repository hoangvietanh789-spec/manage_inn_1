...
def tinhtien(*month_input):
    ...
    df['notify'] = df.apply(lambda row: row['notify'] if row['due_amount'] not in [0,'0'] else row['notify'][row['notify'].find("- Tiền điện:"):row['notify'].find("đ/m3")], axis = 1) # bỏ notify các dòng đẫ thanh toán
